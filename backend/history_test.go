...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{AvgMonthlyPurchase: 8000, PctOfMonthly: 50, LatePayments: 2, PenaltyPerLate: 500})
	saved, err := store.Save(Record{Input: in, Headline: 3000, Label: "credit-ok"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "credit-ok" {
